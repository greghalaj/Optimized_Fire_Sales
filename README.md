...
| `RESPONSEMODE` | Bank response ordering: `0` = independent (i.e., in each round of the totannement, each bank transacts independently of other banks; in other words, they transact in parallel and only after that the prices of the aggregated selling/ buying reacts), `1` = synchronous (i.e., banks in a preselected order, from 0 to $N-1$, transact and in each round of the tanonnement, each bank considers the impact of transactions of all banks that already transacted in this round, `2` = random (similar to 1, but the order of transacting banks is randomised, i.e., permutation of $[0,...,N-1]$) |
...
| `IF_LOAD_FROM_PICKLE` | Load a previously saved stable system from disk (`1`) or generate a fresh one (`0`) |
| `IFSTARTFROMSTEADY` | Start the funding shock experiment from the steady state under no stress (`1`) |
| `NRUNSTOSTABILIZE` | Number of pre-shock runs used to stabilize the system (i.e., to arrive at or close to the steady state) |
